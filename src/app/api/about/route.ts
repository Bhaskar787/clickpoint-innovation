...
export async function GET() {
  try {
    ...
    return NextResponse.json(
      { success: false, error: "Failed to fetch about page content", fallback: DEFAULT_ABOUT_DATA },
      { status: 500 }
    );
  }
}
...
export async function PUT(req: Request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "About page content saved to database!",
      data: updated.content,
    });
  } catch (error) {
    console.error("PUT /api/about error:", error);
    return NextResponse.json(
      { success: false, error: "Failed to save about page content" },
      { status: 500 }
    );
  }
}
